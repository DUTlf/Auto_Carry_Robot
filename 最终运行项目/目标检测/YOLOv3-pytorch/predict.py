...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------------------------#
    #   mode用于指定测试的模式：
    #   'predict'表示单张图片预测, 'predictAllAndSave' 预测所有 并保存结果
    #   'video'表示视频检测
    #   'fps'表示测试fps
    #-------------------------------------------------------------------------#
    mode = "predictAllAndSave"
    # mode = "video"
    # mode = "predict"
    #-------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        while True:
            ...
            try:
                ...
                r_image.show()
                # path="./result/"
                # r_image.save(path+img)

                # print(mylabel, mylocation)

    elif mode == "predictAllAndSave":
        image_ids = open('result/test.txt').read().strip().split()

        for image_id in tqdm(image_ids):
            image_path = "./img/" + image_id + ".jpg"
            image = Image.open(image_path)
            r_image= yolo.detect_image(image)
            r_image.save("./result/" + image_id + ".jpg")

    elif mode == "video":
        capture=cv2.VideoCapture(video_path)
        # capture = cv2.namedWindow('camera', flags=cv2.WINDOW_NORMAL | cv2.WINDOW_FREERATIO)
        if video_save_path!="":
        ...
        test_interval = 100
        img = Image.open('Predict/img/street.jpg')
